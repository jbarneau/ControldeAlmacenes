...
Public Class PCOMP001
        ...
        Try
            If DataGridView1.RowCount <> 0 Then
                Try
                    ...
                    If _usr.Activar_BT("PCONF001") = False Then
                        oc.grabar_cab_OC(_Nremito, Date.Now, _usr.Obt_Usr, _proveedor, 1, _contrato)
                        'GRABAR DETELLE
                        For I = 0 To Me.DataGridView1.RowCount - 1
                            _material = Me.DataGridView1.Item(0, I).Value
                            _cant = Me.DataGridView1.Item(3, I).Value
                            oc.grabar_det_oc(_Nremito, _material, _cant, 0, False)
                        Next
                    ...
                    Else
                        ...
                        For I = 0 To Me.DataGridView1.RowCount - 1
                            _material = Me.DataGridView1.Item(0, I).Value
                            _cant = Me.DataGridView1.Item(3, I).Value
                            oc.grabar_det_oc(_Nremito, _material, _cant, 0, True)
                        Next
                        Dim PANTALLA As New PMAIL001
                        Dim direccion As String
                        PANTALLA.ShowDialog()
                        ...
                        If res = System.Windows.Forms.DialogResult.Yes Then
                            ...
                            PrintDocument1.Print()
                            oc.IMPRIMIR_OC(_Nremito)
                        End If
                        'AGREGADO 29/08/2019
                        'If Not IO.File.Exists("\\192.168.1.222\Almacenes\OCs\" + _Nremito + "-" + DateTime.Today.ToString("dd-MM-AAAA") + "-" + CB_PROVEEDOR.Text) Then
                        'IO.File.Create("\\192.168.1.222\Almacenes\OCs\" + _Nremito + "-" + DateTime.Today.ToString("dd-MM-AAAA") + "-" + CB_PROVEEDOR.Text)
                        'End If
                        Borrar()
                        MENSAJE.MADVE001()
                    End If
                        ...
                        If Metodos.Contar_Coma_Punto(var) <= 1 Then
                            ...
                            If Metodos.Tiene_Decimal(_material, var) = True Then
                                ...
                                If CDec(var) > 0 Then
                                    ...
                                    DataGridView1.Rows.Add(_material, CB_Material.Text, lbUnidad.Text, CDec(var))
                                    TextBox1.Text = Nothing
    ...
    Private Sub PrintDocument1_PrintPage_1(ByVal sender As System.Object, ByVal e As System.Drawing.Printing.PrintPageEventArgs) Handles PrintDocument1.PrintPage
        ...
        Dim R3_T1 As String = ""
        Dim R3_T2 As String = ""
        'Dim cuit As String = CB_PROVEEDOR.SelectedValue.ToString
        Dim DIRECCION As String = oc.Dir_proveedor(_proveedor).ToString
        ...
        R1_T1 = "PROVEEDOR: " + CB_PROVEEDOR.Text.ToString + "(" + _proveedor.ToString + ")"
        R1_T2 = "CONTRATO: " + "(" + _contrato.ToString + ")" + CB_contrato.Text.ToString
        R2_T1 = "DIRECCION: " + DIRECCION.ToString
        R3_T1 = "APROBO: " + oc.QUIEN_APROBO(_Nremito).ToString + "(" + Date.Today.ToString + ")"
        R3_T2 = "CONFECCIONO: " + oc.QUIEN_CONFECCIONO(_Nremito).ToString
        'DEFINO LA LINEA DEL REMITO Y EL SALTO
        Dim LINEA As Integer = 356
        Dim SALTO As Integer = 24
        'IMAGEN ######################################
        e.Graphics.DrawImage(OC_IMAGEN, 0, 0, 800, 1140)
        ...
        e.Graphics.DrawString(Date.Today.ToString, New Font("ARIAL", 12, FontStyle.Regular), Brushes.Black, 435, 101)
