...
#Region "botones"
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BTnuevo.Click
        ...
        Dim consulta As New SqlClient.SqlCommand("SELECT MAX(F_ALTA_003) AS Expr2 FROM dbo.M_PERS_003 WHERE (DEPO_003 = 1)", cnn1)
        obj = consulta.ExecuteScalar()
        If Not IsDBNull(obj) Then
            FechaAux = Convert.ToDateTime(obj)
        End If
        Dim consulta1 As New SqlClient.SqlCommand("SELECT NDOC_003 FROM dbo.M_PERS_003 WHERE (DEPO_003 = 1) AND (F_ALTA_003 = @D1) GROUP BY NDOC_003", cnn1)
        consulta1.Parameters.AddWithValue("D1", FechaAux)
        Dim deposito As SqlDataReader = consulta1.ExecuteReader()
